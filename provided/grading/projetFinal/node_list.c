...
error_code node_list_add(node_list_t *list, node_t node)
{
    ...
    if (list->size + 1 > list->allocated_size) {
        ...
        if (nodes == NULL) {
            ...
            /* CORRECTEUR: Again, this `else` is a bit superfluous, as the `if`
             * part returns from the function. It's also a bit irritating,
             * because the `+= ALLOCATED_MEMORY_LIST` should go together with
             * the line below.
             * [-0.1 style]
             */
        }
    ...
    /* CORRECTEUR: node.sha is not a string, but just an array of chars (and
     * thus strlen will likely blow up)
     * [-1]
     */
...
void node_list_free(node_list_t *list)
{
    ...
    list->nodes = NULL;
    /* CORRECTEUR: as node_list_new allocates a node_list_t structure,
     * node_list_free should free it (symmetry) [-1]
     */
}
