...
error_code node_init(node_t *node, const char *ip, uint16_t port, size_t _unused node_id)
{
    ...
    /* CORRECTEUR: what if calloc fails? [-0.8] */
    node->id = node_id;
    ...
    strcat(dest, id_str);
    /* CORRECTEUR:
     * sprintf(dest, "%s %d %d", ip, port, id);  // or better: snprintf
     */
...
int node_cmp_sha(const node_t *first, const node_t *second)
{
    ...
    /* CORRECTEUR: sha's are not strings but just byte/char arrays, and can thus
     * not be compared with strcmp (this will very likely blow up in your nose)
     * [-0.5]
     */
}
...
int node_cmp_server_addr(const node_t *n1, const node_t *n2)
{
    ...
    /* CORRECTEUR: so n1 address < n2 address == 0? that doesn't seem right...
     * [-0.5]
     */
}
