...
void client_end(client_t *client)
{
    ...
    /* CORRECTEUR: client->server is not on the heap (or only part of the
     * client), and cannot be freed like that - see the comment below
     * [-0.5]
     */
}
...
error_code client_init(client_init_args_t client_init_args)
{
    ...
    if (ring == NULL)
        return ERR_BAD_PARAMETER;
    /* CORRECTEUR: should check right after ring_alloc [-1] */


    if (client_init_args.client->args->N > ring->size)
        client_init_args.client->args->N = ring->size;
        /* CORRECTEUR: what about R and W? [-0.5] */


    client_init_args.client->server = *ring;
    /* CORRECTEUR: why not just assign the pointer? the reference is lost at the
     * end of this function [-0.5]
     */
    int s = get_socket(1);
    if (s == -1) {
        return ERR_NETWORK;
