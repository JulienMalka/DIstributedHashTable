...
error_code client_init(client_init_args_t client_init_args)
{
    ...
    if(args_opt == NULL) {
        ...
        /* CORRECTEUR: Consider letting parse_opt_args set the default and
         * always return a non-NULL args_opt, to simplify this code.
         */
    } else {
    ...
    /* CORRECTEUR: Adapt R and W as well [-0.5 correctness] */

    client_init_args.client->server = *nodes;
    client_init_args.client->socket = get_socket(1);
    /* CORRECTEUR: If get_socket fails? [-1 robustness] */
