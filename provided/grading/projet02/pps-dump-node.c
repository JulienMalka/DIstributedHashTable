...
size_t parse_nbr_kv_pair(char* in_msg)
{
    ...
    /* CORRECTEUR: Consider using memcpy and ntohl here - this is veeeeeery
     * fragile.
     */
}
...
int main(int argc, char* argv[])
{
    ...
    /* CORRECTEUR:                          in_msg + sizeof(uint32_t) */

    if (parsed_kv_pairs == -1) {
    /* CORRECTEUR: See comment about returning -1 below. */
        printf("FAIL\n");
        return -1;
    }
    ...
    if (parsed_kv_pairs < kv_list->size) {
        ...
        if (parsed_kv_pairs != kv_list->size) {
        /* CORRECTEUR: Should accept arbitrary number of packets (not only 2)
         * [-1 correctness]
         */
            printf("FAIL\n");
            return -1;
...
size_t parse_kv_pairs(char* in_msg, size_t length, size_t starting_index, kv_list_t* kv_list)
{
    ...
    /* CORRECTEUR: Why is < 4 wrong? "k\0v" is 3 bytes and should be valid.
     * [-1 correctness]
     * Also, size_t is unsigned, so you cannot return negative values (this gets
     * casted to the equivalent of "size_t max", but it may still blow up).
     * [-0.5 compilation]
     */
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        /* CORRECTEUR: Geeee... seriously, please consider memcpy and/or
         * strncpy! (see my comment in CORRECTEUR.txt)
         */
    }
    value[value_index] = '\0';
    kv_list->list[list_index] = create_kv_pair(key, value);
