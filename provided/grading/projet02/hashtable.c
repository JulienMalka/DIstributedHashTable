...
Htable_t construct_Htable(size_t size)
{
    ...
    /* CORRECTEUR: What if calloc fails? [-1 robustness] */
...
struct bucket* create_bucket(kv_pair_t key_value, struct bucket* next)
{
    ...
    if (new->next == NULL)
    /* CORRECTEUR: Clean up new? [-0.5 memory leak (add_Htable_value)] */
        return NULL;

    new->next = next;
    /* CORRECTEUR: If it's not NULL... overwrite it??  ò_ô
     * [-1 memory leak/correctness (add_Htable_value)]
     * (yes, I did not spot this the last time, I'm sorry)
     * ((also, you seem to call create_bucket only with next == NULL... uhm))
     */
    return new;
}
...
kv_pair_t create_kv_pair(pps_key_t key, pps_value_t value)
{
    ...
    /* CORRECTEUR: Or just strncpy()
     * CORRECTEUR: Or just strdup()
     */
...
void delete_bucket(struct bucket* bck)
{
    if (bck == NULL) {}
        ...
        free(bck->next);
        /* CORRECTEUR: Freeing NULL?? (not deducting points this time, because
         * technically, this compiles correctly - but it's still very
         * unnecessary)
         */
...
/* CORRECTEUR: Somewhat excessive use of empty lines - consider using empty
 * spaces to semantically separate different parts of the code, not to just
 * "stretch" it out. [-0.1 style]
 */
...
void kv_list_free(kv_list_t *list)
{
    ...
    free(list);
    list->list = NULL;
    /* CORRECTEUR: This last line may crash (because `list` is now a destroyed
     * structure). [-0.4 execution]
     */
}
...
kv_list_t *get_Htable_content(Htable_t table)
{
    ...
    /* CORRECTEUR: What if either (or both) of malloc and/or calloc fail?
     * [-1 robustness]
     */
