...
struct bucket* create_bucket(kv_pair_t key_value, struct bucket* next){
	...
	/* CORRECTEUR: What if malloc fails? [-1 robustness] */
...
kv_pair_t copy_kv_pair(pps_key_t key, pps_value_t value){
	...
	/* CORRECTEUR: Should be `strlen(...) + 1` (terminating 0-byte)
	 * [-0.5 correctness add_Htable_value]
	 */
...
void delete_bucket(struct bucket* bck){
	if (bck == NULL){}
		...
		free(bck->next);
		/* CORRECTEUR: Freeing NULL?? [-0.5 correctness] */
		kv_pair_free(&bck->key_value);
	} else {
		delete_bucket(bck->next);
		kv_pair_free(&bck->key_value);
	}
	/* CORRECTEUR: What if `bck->key_value.key == NULL` and
	 * `bck->next != NULL`? [-0.5 robustness]
	 */
}
...
error_code add_value_to_bucket(struct bucket* bck, pps_key_t key, pps_value_t value){
	...
	if (key_value.key == NULL && key_value.value == NULL){
		...
		/* CORRECTEUR: What if create_bucket returns NULL for failure?
		 * [-0.5 robustness (because already deducted in create_bucket)]
		 */
		bck->next = bucket_new;
