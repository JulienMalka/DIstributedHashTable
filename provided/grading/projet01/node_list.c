...
node_list_t *node_list_new()
{
    ...
    /* CORRECTEUR: Magic numbers [-0.5 malus] */
    return new;
}
...
node_list_t *get_nodes()
{
    ...
    char ip[15];
    /* CORRECTEUR: Missing space for terminating zero
     * [-0.5 correctness]
     */
    int index = 0;
    uint16_t port = 0;
...
error_code node_list_add(node_list_t *list, node_t node)
{
    ...
    /* CORRECTEUR: If `realloc` succeeds, `nodes` should be assigned to
     * `list->nodes` (in case the address changes because the memory had to be
     * relocated).
     * [-0.5 correctness (because at least you thought of using a separate
     * pointer)]
     * CORRECTEUR: If `realloc` fails, `list->size` would be 1 too large.
     * Consider moving the `list->size++` from above after the check.
